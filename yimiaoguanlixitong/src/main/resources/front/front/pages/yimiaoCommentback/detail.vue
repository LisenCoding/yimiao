	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.yimiaoCommentbackId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('yimiaoCommentback', this.yimiaoCommentbackId);
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('yimiaoCommentbackYuyue', {
									yonghuId: _this.user.id,
									yimiaoCommentbackId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/yimiaoCommentbackYuyue/list');
							}
						}
					});
				},
