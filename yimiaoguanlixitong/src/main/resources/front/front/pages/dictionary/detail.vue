	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('dictionary', this.dictionaryId);
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('dictionaryYuyue', {
									yonghuId: _this.user.id,
									dictionaryId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/dictionaryYuyue/list');
							}
						}
					});
				},
