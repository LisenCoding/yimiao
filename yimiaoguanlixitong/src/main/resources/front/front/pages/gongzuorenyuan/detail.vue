	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.gongzuorenyuanId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('gongzuorenyuan', this.gongzuorenyuanId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.gongzuorenyuanPhoto ? this.detail.gongzuorenyuanPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('gongzuorenyuanYuyue', {
