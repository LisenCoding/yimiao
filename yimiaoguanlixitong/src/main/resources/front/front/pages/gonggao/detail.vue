<template>
			<view style="margin: 0 30rpx;box-shadow:0 0px 0px 1rpx #a6a6a6">
				<view class="title">
					{{detail.gonggaoName}}
				</view>
				<view style="font-weight: bold;font-size: 20upx;margin:0 30rpx 5rpx;"  :style="(detail.gonggaoValue == null || detail.insertTime == null)?'text-align: center;':''">
					<text v-if="detail.gonggaoValue">
						{{detail.gonggaoValue}}
					</text>
					<text :style="detail.gonggaoValue?'float: right':''">
						{{detail.insertTime}}<!-- .split(' ')[0] -->
					</text>
				</view>
				<view style="box-shadow: 0 0  0 1rpx #a6a6a6;margin: 0 30rpx"></view>
                <viwe>
					<image :src="baseUrl+detail.gonggaoPhoto"></image>
                </viwe>
				<view class="content">
					<rich-text :nodes="detail.gonggaoContent"></rich-text>
				</view>
			</view>
	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('gonggao', this.gonggaoId);
                    this.detail = res.data
					this.detail.gonggaoContent = this.detail.gonggaoContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.gonggaoPhoto ? this.detail.gonggaoPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('gonggaoYuyue', {
